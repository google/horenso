...
sub new {
    ...
    return bless {
        ...
        ordered_key_values => $ordered_key_values,
    }, $class;
}
...
sub _register_finding_empty {
    ...
    for (1..length($self->{ordered_key_values})) {
...
sub increment_key {
    ...
    my $value = $self->{key_to_number}->{$$key};
    $$key = $self->number_to_key($value + $amount);
}

sub number_to_key {
    my ($self, $value) = @_;
    $value %= length($self->{ordered_key_values});
    return substr($self->{ordered_key_values}, $value, 1);
}
...
sub register_autogenerated_codes {
    ...
    for my $key (keys %{$self->{codes}}) {
        ...
        while (my ($number, $kana) = each %number_substs) {
            ...
            $new_key =~ s/;//;
            $self->write_code($new_key, $kana . $self->lookup($key));
        }
    }
